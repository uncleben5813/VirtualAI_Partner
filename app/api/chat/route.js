...
const moods = ["happy", "caring", "playful", "calm", "upset"];
...
function parseAssistant(text) {
  ...
  let mood = null;
  const moodMatch = raw.match(/\[MOOD:\s*(happy|caring|playful|calm|upset)\]/i);
  ...
  const textWithoutTags = raw
    .replace(/\[MOOD:\s*(?:happy|caring|playful|calm|upset)\]/gi, "")
    .replace(/\[MEMORY:\s*[^\]]+\]/gi, "")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
...
- At the very end, add exactly one mood tag: [MOOD: happy], [MOOD: caring], [MOOD: playful], [MOOD: calm], or [MOOD: upset].
